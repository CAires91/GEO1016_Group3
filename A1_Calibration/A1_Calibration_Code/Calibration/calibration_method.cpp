    ...
    Vector3D &t) /// output：a 3D vector encoding camera translation. {
{
    ...
    // initialise needed matrices with 0s
    Matrix U(m, m, 0.0);
    Matrix S(m, n, 0.0);
    Matrix V(n, n, 0.0);
    ...
    // check if the M matrix is correct
    ...
    // calculate sin_theta = sqrt(1 - cos(ø)^2)
    ...
    // t.z needs to be positive, when negative: change the sign of rho
    ...
    if (t.z() < 0) {
        rho = -rho;
        t = rho * inverse(K) * b;
        std::cout << "t.z is negative, changing the sign of rho" << std::endl;
    }
    ...
    for (size_t i = 0; i < points_3d.size(); ++i) {
        ...
        // calculate the vector of the points
